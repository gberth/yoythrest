...
export default class Message {
  ...
  setRequestData(stream: any, request_id: string, requestType: string): void {
    if (!this.message_data.request_data) {
      // @ts-ignore
      this.message_data.request_data = {};
    }
...
export function message(msg_data: {
  [key: string]: string;
}): (x0: { message_data: any; payload: Payload }) => Message {
  return function ({ message_data, payload }): Message {
    return new Message({
